...
def train_direct_joint(model, trainloader1, trainloader2, targetloader, cfg):
    ...
    trainloader_iter1 = enumerate(trainloader1)
    ...
    for i_iter in tqdm(range(cfg.TRAIN.EARLY_STOP)):
        ...
        # UDA Training
        # train on source
        _, src_batch1 = trainloader_iter1.__next__()
        image_src1, label_src1, _, _ = src_batch1
        _, src_batch2 = trainloader_iter2.__next__()
        image_src2, label_src2, _, _ = src_batch2
        # train on target
        _, trg_batch = targetloader_iter.__next__()
        image_trg, label_trg, _, _ = trg_batch

        _, pred_src1 = model(image_src1.to(device), None)
        loss_seg_src1 = loss_calc(pred_src1, label_src1, None, device)
        loss_seg_src1.backward()

        _, pred_src2 = model(image_src2.to(device), None)
        loss_seg_src2 = loss_calc(pred_src2, label_src2, None, device)
        loss_seg_src2.backward()
        ...
        current_losses.update({'loss_seg_src1': loss_seg_src1})
        current_losses.update({'loss_seg_src2': loss_seg_src2})
        print_losses(current_losses, i_iter)
...
def train_scale_source_loss(model, trainloader1, trainloader2, targetloader, cfg):
    ...
    trainloader_iter1 = loopy(trainloader1)
    trainloader_iter2 = loopy(trainloader2)
    targetloader_iter = loopy(targetloader)
    for i_iter in tqdm(range(cfg.TRAIN.MAX_ITERS)):
        _train_scale_source_loss_step(trainloader_iter1, trainloader_iter2, targetloader_iter, i_iter,
                                    model, optimizer, viz_tensorboard, writer, device, cfg)


def _train_scale_source_loss_step(trainloader_iter1, trainloader_iter2, targetloader_iter, i_iter,
                                model, optimizer, viz_tensorboard, writer, device, cfg):
    ...
    src_batch1 = next(trainloader_iter1)
    image_src1, label_src1, _, _ = src_batch1
    src_batch2 = next(trainloader_iter2)
    image_src2, label_src2, _, _ = src_batch2
    ...
    # train on source 1
    _, pred_src1 = model(image_src1.to(device), None)
    loss_seg_src1 = torch.nn.CrossEntropyLoss(weight=None, reduction='none', ignore_index=255)(pred_src1, label_src1.long().to(device))
    loss_seg_src1 *= cfg.TRAIN.SCALE_LOSS

    loss_seg_src1 = loss_seg_src1.mean()
    loss_seg_src1.backward()
    current_losses = {'loss_seg_src1': loss_seg_src1.item()}

    # train on source 2
    _, pred_src2 = model(image_src2.to(device), None)
    loss_seg_src2 = torch.nn.CrossEntropyLoss(weight=None, reduction='none', ignore_index=255)(pred_src2, label_src2.long().to(device))
    loss_seg_src2 *= cfg.TRAIN.SCALE_LOSS

    loss_seg_src2 = loss_seg_src2.mean()
    loss_seg_src2.backward()
    current_losses = {'loss_seg_src2': loss_seg_src2.item()}
...
def train_W_with_FCN(model, trainloader1, trainloader2, targetloader, cfg):
    ...
    for i_iter in tqdm(range(cfg.TRAIN.MAX_ITERS)):
        ...
        src_batch1 = next(trainloader_iter1)
        image_src1, label_src1, _, _ = src_batch1
        label_one_hot_src1 = F.one_hot(label_src1.clamp(0,19).long(), num_classes=20)
        label_one_hot_src1 = label_one_hot_src1.transpose(2, 3).transpose(1, 2).contiguous()
        # source 2
        src_batch2 = next(trainloader_iter2)
        image_src2, label_src2, _, _ = src_batch2
        label_one_hot_src2 = F.one_hot(label_src2.clamp(0,19).long(), num_classes=20)
        label_one_hot_src2 = label_one_hot_src2.transpose(2, 3).transpose(1, 2).contiguous()
        # concat source 1 and source 2
        j = 0 if i_iter % 2 == 0 else 1
        image_src = torch.cat((image_src1, image_src2), dim=0)[j].unsqueeze(0)
        label_src = torch.cat((label_src1, label_src2), dim=0)[j].unsqueeze(0)
        label_one_hot_src = torch.cat((label_one_hot_src1, label_one_hot_src2), dim=0)[j].unsqueeze(0)
...
def train_FCN_with_W(model, trainloader1, trainloader2, targetloader, cfg):
    model.load_state_dict(torch.load(cfg.TRAIN.MODEL_PATH))
    ...
    for i_iter in tqdm(range(cfg.TRAIN.MAX_ITERS)):
        ...
        label_one_hot_src2 = label_one_hot_src2.transpose(2, 3).transpose(1, 2).contiguous()
        # concat source 1 and source 2
        image_src = torch.cat((image_src1, image_src2), dim=0)
        label_src = torch.cat((label_src1, label_src2), dim=0)
        label_one_hot_src = torch.cat((label_one_hot_src1, label_one_hot_src2), dim=0)
        ...
        pred_src1 = pred[0].unsqueeze(0)
        pred_src2 = pred[1].unsqueeze(0)
        pred_src = torch.cat((pred_src1, pred_src2), dim=0)
        pred_trg = pred[2].unsqueeze(0)
...
def train_domain_adaptation(model, trainloader1, trainloader2, targetloader, cfg):
    if cfg.TRAIN.DA_METHOD == 'TargetOnly':
        train_target_only(model, targetloader, cfg)
    elif cfg.TRAIN.DA_METHOD == 'DirectJoint':
        train_direct_joint(model, trainloader1, trainloader2, targetloader, cfg)
    elif cfg.TRAIN.DA_METHOD == 'train_W_with_FCN':
        train_W_with_FCN(model, trainloader1, trainloader2, targetloader, cfg)
    elif cfg.TRAIN.DA_METHOD == 'train_FCN_with_W':
        train_FCN_with_W(model, trainloader1, trainloader2, targetloader, cfg)
    elif cfg.TRAIN.DA_METHOD == 'ScaleSourceLoss':
        train_scale_source_loss(model, trainloader1, trainloader2, targetloader, cfg)
    else:
