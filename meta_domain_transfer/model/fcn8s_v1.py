...
def get_fcn8s_vgg(num_classes=19, init_weights='../../pretrained_models/vggfcn_init_v1.pth', restore_from=None):
    model = FCN8s(num_classes=num_classes)
    if init_weights is not None:
        model.load_state_dict(torch.load(init_weights, map_location=lambda storage, loc: storage))
    if restore_from is not None:
...
def init_vggfcn():
    ...
    params['score_fr.weight'] = vgg16_pretrained['classifier.6.weight'][:19].view(params['score_fr.weight'].size())
    params['score_fr.bias'] = vgg16_pretrained['classifier.6.bias'][:19]
